import { stringify, adds, BOCF, BOCFNode, clone, cmp, muls, os, pows, psis, pred } from "./bocf.js";

const pO = psis(os());
const pOxx3 = psis(pows(os(), pows(os(), pows(os(), os()))));
const e0 = { op: "v", val: [0, 1] };
export function bocf2veblen(bocf: BOCF) {
    let cm = cmp(bocf, pO);
    if (cm === 0) return e0;
    if (cm === -1) return bocf;
    if ((bocf as BOCFNode).op === "i") return bocf;
    if ("+*^".includes((bocf as BOCFNode).op)) {
        const c = clone(bocf as BOCFNode);
        c.val = c.val.map(e => bocf2veblen(e));
        return c;
    }
    cm = cmp(bocf, pOxx3);
    const o = (bocf as BOCFNode).val[0] as BOCFNode;
    ...
    if (cm === -1) {
        if (o.op === "+") {
            ...
            // psi(. + (a<O)) = psi(.)w^a
            if (idx !== -1) return mulv(bocf2veblen(psis(adds(...o.val.slice(0, idx)))), powv(Infinity, bocf2veblen(adds(...o.val.slice(idx)))));
            let state = null as BOCFNode;
            for (idx = o.val.length - 1; idx >= 0; idx--) {
                const e = o.val[idx] as BOCFNode;
                if (state) {
                    // O = O*1
            ...
            if (power) {
                if (cmp(power, os()) === -1) {
                    ...
                    if (res === 1) return { op: "v", val: [addv(leftPart, bocf2veblen(coeff)), power] };
                    // if a > p(.),  p(. + O^a*b) = phi(b,a-1)
                    if (res === -1 && typeof coeff === "number") coeff--;
                    // if a = p(.),  p(. + O^a*b) = phi(b,a)
                    return { op: "v", val: [bocf2veblen(coeff), power] };
                }
                let cmpres = -1;
                if ((power as BOCFNode).op === "*") {
                    const powerCoeff = muls(...(power as BOCFNode).val.slice(1));
                    if (cmp(powerCoeff, os()) === -1) cmpres = cmp(powerCoeff, leftPartbocf);
                }
                ...
                if (cmpres === -1)
                    res = vset(res, 0, addv(addv(leftPart, val0), isFinite(coeff as number) ? 1 : 0));
                if (cmpres === 0)
                    res = vset(res, 0, addv(val0, isFinite(coeff as number) ? 1 : 0));
                if (cmpres === 1)
...
export function firstTermB2V(o: BOCFNode) {
    if (o.op === "*") {
        let res = firstTermB2V(o.val[0] as BOCFNode);
        if (!res) return;
        let coeff = mulv(...o.val.slice(1));
        if (typeof coeff === "number") coeff--;
        res = vset(res, 0, bocf2veblen(coeff));
        return res;
    }
    // psi(O) = e_0
    if (cmp(o, os()) === 0) return e0;
    // psi(O^a) = phi(a,0)
    if (o.op === "^" && cmp(o.val[0], os()) === 0 && cmp(o.val[1], os()) === -1) {
        const res = o.val[1];
        return { op: "v", val: [0, bocf2veblen(res as BOCF)] };
    }
    if (o.op === "^" && cmp(o.val[0], os()) === 0) {
        ...
        const o1 = o.val[1] as BOCFNode;
        // psi(O^{O^a}) = phi(1@a+1)
        if (o1.op === "^" && cmp(o1.val[0], os()) === 0) {
            if (cmp(o1.val[1], os()) === -1) {
                if (isFinite(o1.val[1] as number)) {
                    const res = { op: "v", val: new Array((o1.val[1] as number) + 1).fill(0) };
                    res.val.push(1);
                    return res;
                } else {
                ...
                // psi(O^{O^(O...)}) = phi(1@(..,..)))
                return { op: "v@", val: [1, bocfPow2VeblenMD(o1.val[1] as BOCFNode)] };
            }
        }
        // psi(O^{Ob}) = phi(b,0,0)
        ...
        const o2 = o1.val[0] as BOCFNode;
        if (o1.op === "*" && o2?.op === "^" && cmp(o2.val[0], os()) === 0) {
            // psi(O^{O^a*b}) = phi(b@a+1)
            if (cmp(o2.val[1], os()) === -1) {
                if (isFinite(o2.val[1] as number)) {
                    const res = { op: "v", val: new Array((o2.val[1] as number) + 1).fill(0) };
                ...
                // psi(O^{O^(O...)*b}) = phi(b@(..,..)))
                return { op: "v@", val: [bocf2veblen(muls(...o1.val.slice(1))), bocfPow2VeblenMD(o2.val[1] as BOCFNode)] };
            }
        }
        ...
        if (o1.op === "+") {
            ...
            // psi(O^(. + (a<O)) = phi(...,a,0)
            if (idx !== -1) {
                let left = firstTermB2V(pows(os(), adds(...o1.val.slice(0, idx))) as BOCFNode);
            ...
            // all same e.g. p(O^aw2+O^aw+O^a)
            if (idx === -1) return firstTermB2V(pows(os(), state) as BOCFNode);
            // p(O^(.+O^a*b)) = phi(., .+b@a+1, .)
            let leftPart = firstTermB2V(pows(os(), adds(...o1.val.slice(0, idx + 1))) as BOCFNode);
            let coeff = mulv(...state.val.slice(1));
            const powerPart = state.val[0] as BOCFNode;
            let power = cmp(powerPart, os()) === 0 ? 1 : (powerPart.op === "^" && cmp(powerPart.val[0], os()) === 0) ? powerPart.val[1] : null;
            if (!power) return;
            ...
            if (powres === -1) {
                power = bocf2veblen(power);
                if (isFinite(power as number)) {
                    return vset(leftPart, (power as number) + 1, bocf2veblen(coeff));
                } else {
                ...
                power = bocfPow2VeblenMD(power as BOCFNode);
                return vset(leftPart, power, bocf2veblen(coeff));
            }
        }
    }
...
function bocfPow2VeblenMD(o1: BOCFNode): BOCFNode {
    if (o1.op === "*") {
        const res = bocfPow2VeblenMD(o1.val[0] as BOCFNode);
        res.val[0] = bocf2veblen(muls(...o1.val.slice(1)));
        return res;
    }
    // O = @(1,0)
    if (cmp(o1, os()) === 0) return { op: "@", val: [1, 1] };
    // O^k = @(1,0)
    if (o1.op === "^" && cmp(o1.val[0], os()) === 0) {
        if (cmp(o1.val[1], os()) === -1) return { op: "@", val: [1, bocf2veblen(o1.val[1])] };
        return { op: "@", val: [1, bocfPow2VeblenMD(o1.val[1] as BOCFNode)] };
    }
    if (o1.op === "+") {
        let idx = o1.val.findIndex(e => (cmp(e, os()) === -1));
        // . + (a<O) = @(..,a)
        if (idx !== -1) {
            let left = bocfPow2VeblenMD(adds(...o1.val.slice(0, idx)) as BOCFNode);
        ...
        let coeff = mulv(...state.val.slice(1));
        if (idx === -1) {
            const res = bocfPow2VeblenMD(state.val[0] as BOCFNode);
            res.val[0] = bocf2veblen(coeff);
            return res;
        }
        // .+O^a*b = @(., .+b@a+1, .)
        let leftPart = bocfPow2VeblenMD(adds(...o1.val.slice(0, idx + 1)) as BOCFNode);
        const powerPart = state.val[0] as BOCFNode;
        let power = cmp(powerPart, os()) === 0 ? 1 : (powerPart.op === "^" && cmp(powerPart.val[0], os()) === 0) ? powerPart.val[1] : null;
        if (!power) return;
        const powres = cmp(power, os());
...
export function cmpv(a: BOCF, b: BOCF): 0 | 1 | -1 {
    ...
    if (a.op === "@") return 1;
    if (b.op === "@") return -1;
    if (a.op === "v" && b.op === "v@") return cmpv(v2vAt(a), b);
    if (a.op === "v@" && b.op === "v") return cmpv(a, v2vAt(b));
    if (a.op === "v@" && b.op === "v@") {
        // step 1
        let ai = 0, bi = 0, res: number;
        for (let i = 0; true; i += 2) {
            let ax = a.val[i + 1];
            const bx = b.val[i + 1] ?? ax;
            ax ??= bx;
            if (ax !== 0 && bx !== 0 && !ax && !bx) return 0;
            // if (ai === bi && ai === a.) return cmpv(va, vb);
            const resx = cmpv(ax, bx);
            if (resx !== 0) {
                // 1@w2 > 2@w+1
                res = resx;
                ai = (resx === 1) ? i : i - 2; bi = (resx === -1) ? i : i - 2;
                break;
            }
            const va = a.val[i] ?? 0;
            const vb = b.val[i] ?? 0;
...
export function powv(...val: BOCF[]): BOCF {
    let [u, v] = val;
    if (u === 0) return 0;
    if (u === 1) return 1;
    if (v === 0) return 1;
    ...
    if (u === Infinity && cmpv(v, e0) !== -1) {
        if ((v as BOCFNode).op[0] === "v") return v;
...
export function mulv(...val: BOCF[]): BOCF {
    let set = val as BOCFNode[];
    // a*(b*c)*d
    val = [];
    ...
    for (let i = val.length - 1; i >= 1; i--) {
        const a = val[i - 1] as BOCFNode;
        const b = val[i] as BOCFNode;
        if (typeof a === "number" && isFinite(a) && typeof b === "number") {
            // 1*2
            val[i - 1] = a * b; val[i] = 1; continue;
        }
        const res = cmpv(a, b);
        if (res === 0) {
            // a*a
            val[i - 1] = powv(a, 2); val[i] = 1;
        } else if (res === -1) {
            if (b.op === "^" && cmp(a, b.val[0]) === 0) {
                // w*w^2 w*w^w
                if (typeof b.val[1] === "number") {
                    val[i - 1] = powv(a, 1 + b.val[1]); val[i] = 1;
                } else {
                    val[i - 1] = b; val[i] = 1;
                }
            }
...
export function addv(...val: BOCF[]): BOCF {
    let set = val as BOCFNode[];
    // a+(b+c)+d
    val = [];
    for (let i = 0; i < set.length; i++) {
    ...
    for (let i = val.length - 1; i >= 1; i--) {
        ...
        if (typeof a === "number" && isFinite(a) && typeof b === "number") {
            // 1+2
            val[i - 1] = a + b; val[i] = 0; continue;
        }

        const res = cmpv(a, b);
        if (res === 0) {
            // a*a
            val[i - 1] = mulv(a, 2); val[i] = 0;
        } else if (res === -1) {
            if (b.op === "*" && cmp(a, b.val[0]) === 0) {
                // w+w2
                if (typeof b.val[1] === "number") {
                    val[i - 1] = mulv(a, 1 + b.val[1]); val[i] = 0; continue;
                }
            }
            // w+w^2
            val[i - 1] = b; val[i] = 0;
...
function v2vAt(v: BOCFNode): BOCFNode {
    const vat = { op: "v@", val: [] };
    for (let i = v.val.length - 1; i >= 0; i--) {
        if (v.val[i]) vat.val.push(v.val[i], i);
    }
    return vat;
}
function vset(v: BOCFNode, idx: BOCF, val: BOCF) {
    if (v.op === "v") {
        if (isFinite(idx as number)) {
            const res = { op: "v", val: v.val.slice(0) };
            while ((idx as number) >= v.val.length) res.val.push(0);
            res.val[idx as number] = val;
            return res;
        } else {
...
function vget(v: BOCFNode, idx: BOCF) {
    if (v.op === "v") {
        if (isFinite(idx as number)) {
            return v.val[idx as number] ?? 0;
        }
    }
    if (v.op === "v@") {
        for (let i = 0; i < v.val.length; i += 2) {
