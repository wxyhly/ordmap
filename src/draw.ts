import { adds, BOCF, BOCFNode, cmp, expand, muls, os, pows, printVal, psis, stringify } from "./bocf.js";
import { rescaleDeltaX, rescaleX, rescaleY } from "./mesure.js";
import { psim } from "./mocf.js";
...
type OrdLabel = { pos: number, ord: BOCF, text: string }
...
export class HandleCanvas {
    ...
    coord: Chunk[] = window.location.search === "?0Y" ? [{ ord: { op: "0Y", val: [1, 5] }, preOrd: 0, preOrdDens: 1, min: 0, max: 1, dens: 1 }] : null
    changeCoord: Chunk = null;
    labels: OrdLabel[] = [];
    selected: OrdLabel = null;
    onsave: () => void;
    ...
    init() {
        ...
        const findSelected = (evX: number) => {
            const pos = evX * window.devicePixelRatio;
            if (isMobile) {
                if (pos < this.canvas.width * 0.1) return;
            }
        ...
        this.canvas.onmousemove = (ev) => {
            ...
            if (!touch) findSelected(ev.clientX);
        }
        let moveStartX = null;
        let moveStartY = null;
        ...
        this.canvas.ontouchend = (ev) => {
            ...
            moveStartX = null;
            moveStartY = null;
            if (touch === "scale") ev.stopPropagation();
            touch = "";
            slide = false;
        }
        ...
        const doScale = (deltaY: number, center: number) => {
        ...
        window.onkeydown = (ev: KeyboardEvent) => {
            if (ev.code === "Space") { slide = true; slideMode = 1; }
            if (ev.shiftKey) { slide = true; slideMode = 2; }
            if (ev.code === "KeyW") { this.maxDepth++; redraw = true; }
            if (ev.code === "KeyS" && this.maxDepth > 1) { this.maxDepth--; redraw = true; }

            if (ev.code === "KeyT") { this.scaleSpeed *= 1.1; if (this.scaleSpeed > 0.015) this.scaleSpeed = 0.015; redraw = true; }
            if (ev.code === "KeyG") { this.scaleSpeed /= 1.1; if (this.scaleSpeed < 0.001) this.scaleSpeed = 0.001; redraw = true; }
        }
        ...
        const loop = () => {
            ...
            if (touch === "scale") {
                if (moveEndX * window.devicePixelRatio > this.canvas.width * 0.1) { slideMode = 2; slide = true; }
            ...
            if (slide) {
                if (!(slideMode === 2 && this.maxDens === this.secondDens)) {
                    ...
                    if (Math.abs((now - target) / target) < 0.01) slide = false;
                    redraw = true;
                } else {
                    slide = false;
                }
    ...
    draw() {
        ...
        // draw fractal
        this.drawOmega(ct,
            coord.ord, coord.preOrd, coord.preOrdDens * this.range[2],
            this.range[0], this.range[1],
            this.range[2], this.maxDepth
        );
        // draw pred
        ...
        if (this.maxDens <= 0) { this.maxDens = preMax; }
        if (this.secondDens <= 0) this.secondDens = this.maxDens;
        // coord convertion
        ...
        if (!fullscreen) {
            ...
            if (max > this.canvas.width && min < (max - min) / this.range[2]) return;
            // zoom out
            const zoom = this.coord.shift();
            if (!zoom?.lastDens) { this.coord.unshift(zoom); return; }
            // f(a') = a
            // f(b') = b
            // f(x) = (x-a')*(b-a)/(b'-a')+a
            // k = (b-a)/(b'-a')
            ...
            // zoom in
            if (!this.changeCoord) return;
            if (this.changeCoord.ord && this.coord[0].ord && cmp(this.changeCoord.ord, this.coord[0].ord) === 0) return;
    ...
    drawOmega(ct: CanvasRenderingContext2D, ord: BOCF, preOrd: BOCF, preOrdDens: number, min: number, max: number, dens: number, it: number) {
        if (it <= 0 || max < 0 || min > this.canvas.width) return;
        const width = max - min;
        ...
        if (max < this.canvas.width) this.drawOrd(ct, max, dens, label.length * 5 < width + 5 ? label : "", ord);
        let cursor = min; // prev drawed pos
        let shift = ord ? (ord as BOCFNode).op === "0Y" ? -1 : this.add1ord.find(e => cmp(e, ord) === 0) ? 1 : this.add0ord.find(e => cmp(e, ord) === 0) ? 0 : -1 : 0;
        let findingShift = true;
        for (let n = 1; n < this.maxIteration; n++) {
            let subOrd: BOCF | null | false = null;
            if (ord !== null) {
                if (!findingShift) subOrd = expand(ord, n + shift);
                while (findingShift) {
            ...
            let nd = ndrel * dens;
            // if too short, stop
            if (nd < this.deltaY) break;
            let npos = min + nv * width;
            // if too tight, skip some bars
            if (npos - cursor < this.deltaX) continue;
            ...
            if (subOrd && nd < this.deltaTextY && n > 1 && width < this.trySkipWidth) {
                ...
                while (snd > rescaleX[1]) {
                    ...
                    while (true) {
                        ...
                        if (nshift > 10) {
                            console.error(printVal(preOrd), printVal(subsub)); break;
                        }
                    }
                    sub = subsub as BOCF;
                }
            ...
            this.drawOmega(ct, !sparse ? null : subOrd as BOCF, preOrd, preOrdDens, startPos, npos, nd, it - 1);
            if ((subOrd as any)?.val && (subOrd as any)?.val[1]?.val && (subOrd as any)?.val[1]?.op === "p" && (subOrd as any)?.val[1]?.val[0] === Infinity && (subOrd as any)?.val[1]?.val[1] === 0) {
                console.log("oma");

            }
            preOrd = subOrd as BOCF;
            preOrdDens = nd;
            cursor = npos;
        }
    ...
    drawOrd(ct: CanvasRenderingContext2D, pos: number, dens: number, label: string, ord: BOCF) {
        ...
        if (dens < this.maxDens) {
            if (dens > this.secondDens) this.secondDens = dens;
        } else {
        ...
        if (label.length > 200) label = label.slice(0, 96) + " .... " + label.slice(-96);
        ...
        if (ord || ord === 0) this.labels.push({ pos, ord, text: label });
