...
export class HandleCanvas {
    ...
    draw() {
        ...
        // draw pred
        if (this.range[0] > 0 && coord.preOrdDens) this.drawOrd(ct, this.range[0], coord.preOrdDens * this.range[2], printVal(coord.preOrd), coord.preOrd);
        ct.stroke();
        // draw selected
        if (this.selected) {
            ct.strokeStyle = "red";
    ...
    drawOmega(ct: CanvasRenderingContext2D, ord: BOCF, preOrd: BOCF, preOrdDens: number, min: number, max: number, dens: number, it: number) {
        ...
        let shift = ord ? (ord as BOCFNode).op === "0Y" ? -1 : this.add1ord.find(e => cmp(e, ord) === 0) ? 1 : 0 : 0;
