...
export function bocf2mocf(bocf, order) {
    const order_succ = isFinite(order) ? order + 1 : order;
    if (cmp(bocf, psis(os(order_succ))) === -1)
        return bocf;
    ...
    if (o.op === "p") {
        ...
        const o1 = bocf.val[0];
        if (o1.op === "+") {
            let idx = o1.val.findIndex(e => (cmp(e, os(isFinite(order) ? order + 1 : order)) === -1));
            // psi_n(. + (a<O_n_1)) = psi_n(.)w^a
            if (idx !== -1)
                return mulv(bocf2mocf(psim(adds(...o1.val.slice(0, idx)), order)), powm(Infinity, bocf2mocf(adds(...o1.val.slice(idx)))));
            const om = clone(o1);
            const terms = [];
            idx = o1.val.length - 1;
            while (idx >= 0) {
                ...
                if (state.op === "*")
                    terms.unshift(mulv(...state.val)); //avoid (#*1)
                else
                    terms.unshift(state);
            }
            const dealed = [];
    ...
    if (o.op === "o")
        return os(bocf2mocf(o.val[0]));
    return bocf;
}
function dealterm(o, order, minusOne, prevTerms) {
    if (o.op === "o") {
        const pre = isFinite(o.val[0]) ? o.val[0] - 1 : o.val[0];
        // O=>0 || +O => +1
        if (!isFinite(o.val[0]))
            return os(bocf2mocf(o.val[0]));
        if (pre !== -1 && cmp(pre, order) === 0)
            return minusOne ? 0 : 1;
        // O_n=>O_n
        if (!minusOne) {
            // (#+O_2) => #+P_1(#+1)
            return psim(addv(...prevTerms, 1), isFinite(o.val[0]) ? o.val[0] - 1 : o.val[0]);
        }
        return psim(0, isFinite(o.val[0]) ? o.val[0] - 1 : o.val[0]);
    }
    // if (cmp(o, os()) === 0) return minusOne ? 0 : 1;
    if (o.op === "*") {
        const first = o.val[0];
        if (first.op === "o") {
            const pre = isFinite(first.val[0]) ? first.val[0] - 1 : first.val[0];
            if (!isFinite(first.val[0]))
                return muls(os(bocf2mocf(first.val[0])), ...o.val.slice(1));
            const noPsi = pre !== -1 && cmp(pre, order) === 0;
            // On=>n-1
            // O_2 n=>p_1(n-1)
            const val = bocf2mocf(muls(...o.val.slice(1)));
            if (!minusOne && first.val[0] !== 1) { // (#+O_2*k) => #+P_1(O_2+k)
                return psim(addv(...prevTerms, val), pre);
            }
            if (isFinite(o.val[1])) {
                const val = o.val[1] - (minusOne ? 1 : 0);
                return noPsi ? val : psim(val, pre);
            }
            // Ow=>w
            // Op(O)=>p(0)
            return noPsi ? val : psim(val, pre);
            ...
            return muls(dealterm(o.val[0], 0, minusOne), ...o.val.slice(1).map(e => bocf2mocf(e)));
        }
    }
    if (o.op === "^" && o.val[0].op === "o") {
        // O^n=>O^(n-1)
        if (isFinite(o.val[1]))
            return powm(o.val[0], o.val[1] - 1);
        return bocf2mocf(o);
    }
    if (o.op === "p") {
        // p_1(O_2) => p_1(0)
...
export function psim(val, ord = 0) {
