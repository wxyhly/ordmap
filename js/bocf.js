...
export function stringify(bocf) {
    if (bocf === Infinity)
        return "ω";
    if (typeof bocf === "number")
        return bocf.toString();
    if (bocf.op === "0Y")
        return "Y(" + bocf.val.join(",") + ")";
    if (bocf.op === "i")
        return "EBO";
    if (bocf.op === "+")
    ...
    if (bocf.op === "p")
        return "Ψ_{" + stringify(bocf.val[1]) + "}(" + stringify(bocf.val[0]) + ")";
    if (bocf.op === "o")
        return "Ω_{" + stringify(bocf.val[0]) + "}";
    if (bocf.op === "#")
        return "#"; // dbg only
    // veblens
    if (bocf.op === "v") {
    ...
    if (bocf.op === "v@" || bocf.op === "@") {
        ...
        for (; i < bocf.val.length; i += 2) {
            if (isFinite(bocf.val[i + 1]))
                break;
            maps.push(stringify(bocf.val[i]) + (bocf.val[i + 1] ? "@" + stringify(bocf.val[i + 1]) : ""));
        }
        if (i < bocf.val.length) {
            const keys = new Array(bocf.val[i + 1] + 1);
            for (; i < bocf.val.length; i += 2) {
                keys[bocf.val[i + 1]] = stringify(bocf.val[i]);
            }
...
export function pred(bocf) {
    if (bocf === Infinity || bocf === 0)
        return null;
    if (typeof bocf === "number")
        return bocf - 1;
    if (bocf.op === "o")
...
export function expand(bocf, n) {
    if (bocf === Infinity)
        return n;
    if (typeof bocf === "number")
        return null;
    if (bocf.op === "0Y") {
        const val = expand0Y(bocf.val, n);
        if (!val)
            return null;
        return { op: "0Y", val };
    }
    ...
    if (bocf.op === "p") {
        ...
        if (typeof O === "number")
            return expand(pows(Infinity, O), n);
...
export function clone(exp) {
    return { op: exp.op, val: exp.val.map(e => (typeof e === "number" ? e : clone(e))) };
}
// x+$=>x  x*$=>x  x^$=>x
function removeFixPoint(exp) {
    if (exp.op === "p")
        return psis(removeFixPoint(exp.val[0]), exp.val[1]);
...
function replaceFixPoint(exp, val) {
    if (exp.op === "#") {
        exp.op = val.op;
    ...
    for (const e of exp.val) {
        if (typeof e !== "number")
            replaceFixPoint(e, val);
    }
}
let _fixPointCache = null;
function findFixpoint(O) {
    // hyp: O is nonit
    ...
    if (O.op === "o") {
        ...
        if (pre !== null && pre !== false) {
            ...
            return os(findFixpoint(O.val[0]));
        }
    }
    // if O = a + b is nonit, then b is nonit
    if (O.op === "+")
        return adds(...remained, findFixpoint(inner));
    ...
    if (O.op === "^") {
        if (pre !== null && pre !== false) {
            return findFixpoint(muls(pows(...remained, pre), ...remained));
        }
...
;
export function psis(val, ord = 0) {
    if (val?.op === "o") {
