...
export function calcMountain(s) {
    let lastLayer = [];
    ...
    for (let i = 0; i < length; i++) {
        ...
        if (!isFinite(value) || !Number.isInteger(value) || value < 1)
            value = 1; //throw Error("Invalid sequence");
...
export function expand0Y(s, n) {
    ...
    let cutHeight = 0;
    while (cutHeight + 1 < height && mountain[cutHeight][cutPosition][1])
        cutHeight++;
    ...
    for (let i = 1; i <= n && cutHeight; i++) { //iteration
        for (let x = badRootPosition; x < cutPosition; x++) { //position
            for (let y = 0; y < height; y++) {
                if (x == badRootPosition && y < cutHeight - 1)
                    result[y].push([NaN, mountain[y][cutPosition][1]]);
                else if (!mountain[y][x][1])
                    result[y].push([mountain[y][x][0], 0]);
                else if (mountain[y][x][1] && x - mountain[y][x][1] >= badRootPosition && (x > badRootPosition || y < cutHeight))
                    result[y].push([NaN, mountain[y][x][1]]);
                else
                    result[y].push([NaN, mountain[y][x][1] + (cutPosition - badRootPosition) * i]);
            }
        }
    }
    ...
    for (var x = 0; x < resultLength; x++)
        rr.push(result[0][x][0]);
    return rr;
}
//# sourceMappingURL=yseq.js.map